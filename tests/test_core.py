...
def test_native_yolo_forwards_native_source_and_options() -> None:
    operator = yolo.Predict("custom.pt", task="detect", verbose=True, conf=0.4)
...
def test_yolo_operator_classes_construct_their_own_native_model() -> None:
    assert isinstance(yolo.Predict("custom.pt"), yolo.Predict)
    assert isinstance(yolo.Embed("custom.pt"), yolo.Embed)
    assert isinstance(yolo.Track("custom.pt"), yolo.Track)


def test_yolo_operators_reuse_a_supplied_native_model() -> None:
    model = FakeYOLOModel("custom.pt")
    assert yolo.Predict(model).model is model
    assert yolo.Embed(model).model is model
    assert yolo.Track(model).model is model
...
def test_native_yolo_embed_is_a_dedicated_operator() -> None:
    embedder = yolo.Embed("custom.pt", conf=0.4)
    embeddings = embedder("frame.jpg")
    assert isinstance(embeddings[0], Tensor)
    assert embedder.model.calls == [("embed", "frame.jpg", {"conf": 0.4})]
    with pytest.raises(TypeError, match="yolo.Embed"):
        yolo.Predict(embed=[-2])


def test_native_yolo_call_accepts_data_only() -> None:
    with pytest.raises(TypeError):
        yolo.Predict()("frame.jpg", conf=0.4)  # type: ignore[call-arg]
    with pytest.raises(TypeError):
        yolo.Predict()()  # type: ignore[call-arg]


def test_native_yolo_track_forwards_options_and_persists_at_operator_scope() -> None:
    tracker = yolo.Track("detect.pt", persist=True, tracker="bytetrack.yaml", conf=0.1)
